...
5. **P1: Real guarded HTTP requests failed with the upgraded dispatcher.** Post-deployment validation caught `invalid onRequestStart method`: Node's built-in fetch and the installed Undici Agent used different dispatcher protocols. The previous runtime images were restored while correcting this; fetch and Agent now come from the same Undici package. The regression test exercises real HTTP handlers against a local socket fixture instead of injecting a mocked fetch. Deployment runs this test in the candidate image before migrations or service restarts and stops on failure. Existing DNS pinning, private-address blocking, redirect validation, and timeouts remain enabled.
