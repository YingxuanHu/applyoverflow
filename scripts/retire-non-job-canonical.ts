  ...
  const candidates =
    ...
    where status in ('LIVE', 'AGING', 'STALE')
      and (
        ...
        or title ~* '^(will .* jobs\\??|thanks for applying|current opening|please fill details and apply)$'
        ...
        or description ~* '(not an active opening|building (a )?talent pipeline|\\[(pipeline|talent pool)\\]|talent pool|talent community|general application|open application|expression of interest|submit your (resume|cv)|future opportunities|evergreen (role|opportunity|opening)|explore open roles|widget title goes here|meta text goes here|while you wait to hear back|interview prep tools|ai interview practice|lender[[:space:]]*-[[:space:]]*borrower)'
        or "applyUrl" ~* '/(ai-guidelines|artificial-intelligence|blog|guides?|docs?|events|support|resources?|case-studies|collections|datasets?|insights|media/videos?|models?|news(room)?|partners?|papers?|press(-release)?|protect-yourself|products?|posts|spaces|videos|faqs?|thank-you|download|webinars?|whitepapers?|lesson-center|people-ops|personal-loan)(/|$)'
