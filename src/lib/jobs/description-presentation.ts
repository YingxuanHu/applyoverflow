...
function categorize(heading: string): DescriptionCategory {
  ...
  if (/preferred|nice to have|bonus|atout|souhait/.test(text)) return "preferred";
  ...
  if (/requirement|qualification|what you.*bring|who you are|skills|exigences|profil recherché/.test(text)) return "requirements";
...
export function buildDescriptionPresentation(raw: string) {
  ...
  for (const category of priority) {
    ...
    const text = first.kind === "list" ? first.items[0] : first.kind === "paragraph" ? first.text : "";
