...
export async function upsertJobFeedIndex(canonicalJobId: string) {
  ...
  const canonical = await prisma.jobCanonical.findUniqueOrThrow({
    ...
    include: {
      sourceMappings: {
        ...
        select: { sourceReliability: true, sourceType: true, sourceQualityKind: true },
      },
  ...
  await prisma.$executeRaw`
    ...
    WHERE
      id = ${canonicalJobId}
      AND "updatedAt" = ${canonical.updatedAt}
      AND (
  ...
  const projection = {
    ...
    indexedAt: now,
    updatedAt: now,
  } satisfies Prisma.JobFeedIndexUncheckedCreateInput;
  await prisma.$executeRaw(buildFeedIndexWrite(projection, canonical.updatedAt));
}
