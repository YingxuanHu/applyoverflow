...
test("jobs filters support scoped search, removable chips, compact filters, and salary range", () => {
  ...
  assert.match(pageSource, /JobsNavigationPendingBoundary/);
  ...
  assert.match(searchParamMemorySource, /if \(stateSearch\) \{[\s\S]*inAppSearchParamMemory\.set\(storageKey, stateSearch\);[\s\S]*return;/);
