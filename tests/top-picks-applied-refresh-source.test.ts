...
test("top picks refreshes often and keeps applied jobs visible", () => {
  ...
  assert.match(configSource, /TOP_PICKS_STORE_LIMIT[\s\S]*\?\? 300/);
  assert.doesNotMatch(querySource, /trackedApplications:\s*\{\s*none:/);
  assert.match(serviceSource, /const visibleTopPickWhere/);
  assert.doesNotMatch(serviceSource, /\.\.\.history\.appliedJobIds/);
  assert.match(trackerSource, /queueTopPicksRefreshForApplicationSignal/);
